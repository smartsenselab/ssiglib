...
namespace ssf{
	...
	public:
		...
		CORE_EXPORT Parameter(const std::string& name, const std::string& description, const ParamType& requiredType = ParamType::STRING);
		...
		ParamType getRequiredType() const;
		void setRequiredType(const ssf::ParamType& requiredType);
